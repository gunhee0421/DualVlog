...
const Code: React.FC<Editor1Props> = ({ value, onChange, select }) => {
  ...
  useEffect(() => {
    if (quill.current) {
      const qlToolbar = quill.current.querySelector(
        ".ql-toolbar",
      ) as HTMLElement
      const qlContainer = quill.current.querySelector(
        ".ql-container",
      ) as HTMLElement
  ...
  const handleSelectionChange = () => {
    ...
    if (quill.current) {
      const quillInstance = (
        quill.current.querySelector(".ql-container") as any
      ).__quill
      const selection = quillInstance.getSelection()
      ...
      if (selection && selection.index !== null) {
        ...
        if (format["code-block"]) {
          ...
          const selectedLineIndex = fullText
